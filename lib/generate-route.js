...
const buildRoute = async (tableInfo, path) => {
	let templateName = `${__dirname}/lib/template/route_template.js`;
	let fileName = `${path}/routes/${tableInfo.tableName}_route.js`;

	let routeTemplate = fs.readFileSync(templateName).toString();

	routeTemplate = routeTemplate.replace(/ResourceName/g, tableInfo.resourceName);
	routeTemplate = routeTemplate.replace(/variableName/g, tableInfo.variableName);
	routeTemplate = routeTemplate.replace(/table\_name/g, tableInfo.tableName);
	routeTemplate = routeTemplate.replace(/api-resource/g, tableInfo.apiName);
