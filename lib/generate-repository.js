...
const buildRepository = async (tableInfo, path) => {
	const fs = require("fs");
	let templateName = `${__dirname}/../lib/template/repository_template.js`;
	...
	repoTemplate = repoTemplate.replace(/ResourceName/g, tableInfo.resourceName);
	repoTemplate = repoTemplate.replace(/table\_name/g, tableInfo.tableName);
	repoTemplate = repoTemplate.replace(/RESOURCE\_FIELDS/g, resource_fields);
	repoTemplate = repoTemplate.replace(/TABLE\_INFO/g, JSON.stringify(tableInfo));

	let paramater_list = new Array(Object.keys(tableInfo.columns).length);
	paramater_list.fill("?");

	repoTemplate = repoTemplate.replace(/PARAMATER\_LIST/g, paramater_list.join(","));
